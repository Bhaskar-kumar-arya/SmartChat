...
export class LidPnLinker {
  ...
  async linkLidAndPn(
    ...
    await this.repository.upsertLidMap(cleanLid, cleanPn, source).catch((err: unknown) => {
      console.error('[LidPnLinker] Failed to upsert lidMap entry:', err)
    })

    // 2. Relational Identity Sync
    // Find identities for both
    const lidAlias = await this.repository.findIdentityAlias(cleanLid)
    let pnIdentity = await this.repository.findIdentityByPhoneNumber(cleanPn)
    
    if (!pnIdentity) {
      // Look for PN alias
      const pnAlias = await this.repository.findIdentityAlias(cleanPn)
      if (pnAlias) {
        pnIdentity = await this.repository.findIdentityById(pnAlias.identityId)
      }
    }
    ...
    if (pnIdentity) {
      ...
      await this.repository.upsertIdentityAlias(cleanLid, 'LID', identityId)
      ...
      if (orphanId) {
        const { aliases: aliasCount, messages: msgCount, members: memberCount, reactions: reactionCount } = 
          await this.repository.countIdentityReferences(orphanId)

        if (aliasCount === 0 && msgCount === 0 && memberCount === 0 && reactionCount === 0) {
          await this.repository.deleteIdentity(orphanId).catch((err: unknown) => {
      ...
      await this.repository.updateIdentity(identityId, { phoneNumber: cleanPn })
      await this.repository.upsertIdentityAlias(cleanPn, 'PN', identityId)
    } else {
      // Neither exists, create a new identity and both aliases
      const newId = await this.repository.createIdentity({ phoneNumber: cleanPn })
      identityId = newId.id
      await this.repository.upsertIdentityAlias(cleanPn, 'PN', identityId)
      await this.repository.upsertIdentityAlias(cleanLid, 'LID', identityId)
    }
