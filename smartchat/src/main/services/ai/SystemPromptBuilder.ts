...
export class SystemPromptBuilder implements ISystemPromptBuilder {
  build(
    ...
    // Default fallbacks to prevent breaking first run/empty db cases
    const phoneNum = userDetails?.phoneNumber || '919931386969'
    const lid = userDetails?.lid || '187273727488097'
    const phoneJid = userDetails?.phoneJid || '919931386969@s.whatsapp.net'
    const linkedJid = userDetails?.linkedJid || '187273727488097@lid'
...
${useThinkMode ? reactProtocol : standardProtocol}`.trim()
  }
}
